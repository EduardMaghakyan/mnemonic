...
A macOS menu-bar app that records short voice notes, transcribes them with **Gemma 4 E4B** (native audio via `llama-server`), and appends one bullet per recording to a daily Markdown file. Comes with a `mnemonic` CLI for retrieval.

The model is used for transcription + light cleanup only (filler removal, false-start fixes) — no titles, summaries, or extraction. Notes read like the user wrote them.
...
A new bullet is appended to today's daily note at `~/Mnemonic/notes/YYYY-MM-DD.md`:
...
|---|---|
| `mnemonic ls [--since 7d] [--limit N]` | List daily notes newest-first, with entry count per day |
| `mnemonic find QUERY [--open]` | Case-insensitive substring search across all daily notes; ±1 line of context. `--open` opens the first hit. |
| `mnemonic show [DATE]` | Print a daily note. Defaults to today; accepts `YYYY-MM-DD`. Uses `bat` if available, else plain print. |
...
Mnemonic writes one Markdown file per day, named `YYYY-MM-DD.md` — the same format Obsidian's built-in **Daily Notes** plugin uses. Two ways to wire it up:

1. **Point `notes_dir` at the Daily Notes folder.** In Obsidian: Settings → Daily Notes → note the "New file location" (default: vault root). Then edit `~/.config/mnemonic/config.toml`:

   ```toml
   [paths]
   notes_dir = "~/Documents/YourVault"
   ```

   Mnemonic appends to the same `YYYY-MM-DD.md` file Obsidian opens when you click "Today's daily note". Saves hot-reload — no restart needed.

2. **Symlink `~/Mnemonic/notes` into a vault subfolder.** Keeps `~/Mnemonic/notes` as the source of truth, so the CLI's `ls`/`find`/`show` continue to work against the canonical path:

   ```bash
   ln -s ~/Mnemonic/notes "~/Documents/YourVault/Voice Notes"
   ```

   You'll need to point Obsidian Daily Notes at the `Voice Notes` subfolder for the integration to work.

What you get out of the box:

- Each bullet line is plain Markdown — search, backlinks, and graph view all work.
- The `[audio](...)` link plays inline in Obsidian's audio embed _if_ audio lives inside the vault tree (see "Audio links in Obsidian" below).

Not integrated yet: no auto-open-in-Obsidian after a recording. Possible follow-up.

### Audio links in Obsidian

By default Mnemonic stores audio at `~/Mnemonic/audio/` (sibling of `~/Mnemonic/notes/`), so the link `[audio](../audio/...)` resolves on the filesystem but **not** inside Obsidian — Obsidian only knows about files inside the vault. Two ways to fix:

1. **Symlink the audio dir into the vault** alongside notes:
   ```bash
   ln -s ~/Mnemonic/audio "~/Documents/YourVault/audio"
   ```
2. **Move audio inside notes_dir.** Edit `~/.config/mnemonic/config.toml`:
   ```toml
   [paths]
   audio_dir = "~/Mnemonic/notes/audio"
   ```
   New recordings land at `~/Mnemonic/notes/audio/YYYY-MM-DD/`, and a single notes-dir symlink covers both. (Existing audio files stay where they are; move them manually if you want them findable.)
...
| Note body has third-party narration ("the speaker is…") | The structuring prompt explicitly forbids this voice, but Gemma can still drift on rare audio. The audio is in `~/Mnemonic/audio/YYYY-MM-DD/` if you want to retry the recording. |
